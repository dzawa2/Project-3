...
public class Client extends Application {
    ...
    private Scene createSelectPlayerScene() {
        ...
        for (var entry : planets.entrySet()) {
            ...
            ImageView titleView = new ImageView(new Image(getClass().getResourceAsStream(titlePath)));
    ...
    private Scene createGameScene() throws Exception {
        ...
        chatInput.setOnAction(e -> {
            ...
            if (!text.isEmpty()) {
                try { out.writeObject(new ChatMessage(playerId, text)); out.flush(); chatInput.clear(); }
        ...
        if (obj instanceof GameEvent ge) {
            switch (ge.getType()) {
                case SELECT -> {
                    ...
                    if (ge.getSelectingPlayer().equals(playerId)) myPieceImage = chosen;
                    else                                    oppPieceImage = chosen;
    ...
    private void sendMove(int col) {
        try { out.writeObject(col); out.flush(); }
